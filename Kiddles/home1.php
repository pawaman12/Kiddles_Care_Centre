<!DOCTYPE html>
<html lang="en">
<head>
   <meta charset="UTF-8">
   <meta http-equiv="X-UA-Compatible" content="IE=edge">
   <meta name="viewport" content="width=device-width, initial-scale=1.0">
   <title>Kiddles</title>

   <!-- swiper css link  -->
   <link rel="stylesheet" href="https://unpkg.com/swiper@7/swiper-bundle.min.css" />

    <!--Logo-->
    <link rel="shortcut icon" type="image/png" href="images/kiddles.png">

   <!-- font awesome cdn link  -->
   <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">

   <!-- custom css file link  -->
   <link rel="stylesheet" href="css/style.css">

</head>
<body>
   
<!-- header section starts  -->

<section class="header">

   <a href="home.php" class="logo">kiddles-Care Centre For Kids</a>
      <video autoplay loop muted plays-inline class="video-1">
         <source src="images/video1.mp4" type="video/mp4"> 
         </video> 

   <nav class="navbar">
      <a href="about1.php">about</a>
      <a href="parentsguide.php">parents guide</a>
      <a href="tasktracker.php">task-tracker</a>
      <a href="tasks.php">task-adder</a>
      <a href="kidslogin.php">kids</a>
      <a href="home.php">logout</a>
   </nav>

   <div id="menu-btn" class="fas fa-bars"></div>

</section>

<!-- header section ends -->

<!-- home section starts  -->

<section class="home">

   <div class="swiper home-slider">

      <div class="swiper-wrapper">

         <div class="swiper-slide slide" style="background:url(images/k-1.jpg) no-repeat">
            <div class="content">
               <span>Care, Learning, Fun</span>
               <h3>Let your kids grow with us!</h3>
               <a href="package.php" class="btn">discover more</a>
            </div>
         </div>

         <div class="swiper-slide slide" style="background:url(images/k-2.jpg) no-repeat">
            <div class="content">
               <span>Care, Learning, Fun</span>
               <h3>Whats better than learning with fun?</h3>
               <a href="package.php" class="btn">discover more</a>
            </div>
         </div>

         <div class="swiper-slide slide" style="background:url(images/k-3.jpg) no-repeat">
            <div class="content">
               <span>Care, Learning, Fun</span>
               <h3>Children learn as they play.</h3>
               <a href="package.php" class="btn">discover more</a>
            </div>
         </div>
         
      </div>

      <div class="swiper-button-next"></div>
      <div class="swiper-button-prev"></div>

   </div>

</section>

<!-- home section ends -->

<!-- services section starts  -->

<section class="services">

   <h1 class="heading-title"> our services </h1>

   <div class="box-container">

      <div class="box">
         <img src="images/icon-5.png" alt="">
         <h3>Supervision</h3>
      </div>

      <div class="box">
         <img src="images/icon-6.png" alt="">
         <h3>Care
         </h3>
      </div>

      <div class="box">
         <img src="images/icon-1.png" alt="">
         <h3>Good Environment</h3>
      </div>

      <div class="box">
         <img src="images/icon-2.png" alt="">
         <h3>Social Skills</h3>
      </div>

      <div class="box">
         <img src="images/icon-3.png" alt="">
         <h3>Learning</h3>
      </div>

      <div class="box">
         <img src="images/icon-4.png" alt="">
         <h3>Fun</h3>
      </div>

   </div>

</section>

<!-- services section ends -->

<!-- home about section starts  -->

<section class="home-about">

   <div class="image">
      <img src="images/k-10.jpg" alt="">
   </div>

   <div class="content">
      <h3>about us</h3>
      <p>Kiddles is a learning and progress website for kids to keep up with their hygiene habits, and health care!</p>
      <a href="about.php" class="btn">read more</a>
   </div>

</section>

<!-- home about section ends -->

<!-- section starts  -->

<section class="home-offer">
   <div class="content">
      <h3>Kids Can Enjoy</h3>
      <p> watching fun interactive stories, learn poems and memorize shapes and much more!  </p>
   </div>
</section>

<!-- section ends -->


<!-- footer section starts  -->

<section class="footer">

   <div class="box-container">

      <div class="box">
         <h3>quick links</h3>
         <a href="about.php"> <i class="fas fa-angle-right"></i> about</a>
         <a href="parentsguide.php"> <i class="fas fa-angle-right"></i> parents guide</a>
         <a href="tasktracker.php"> <i class="fas fa-angle-right"></i> task-tracker</a>
         <a href="tasks.php"> <i class="fas fa-angle-right"></i> task addder</a>
         <a href="kidslogin.php"> <i class="fas fa-angle-right"></i> kids</a>
         <a href="home.php"> <i class="fas fa-angle-right"></i> logout</a>
      </div>

      <div class="box">
         <h3>extra links</h3>
         <a href="#"> <i class="fas fa-angle-right"></i> ask questions</a>
         <a href="about.php"> <i class="fas fa-angle-right"></i> about us</a>
         <a href="#"> <i class="fas fa-angle-right"></i> privacy policy</a>
         <a href="#"> <i class="fas fa-angle-right"></i> terms of use</a>
      </div>

      <div class="box">
         <h3>contact info</h3>
         <a href="#"> <i class="fas fa-phone"></i> +123-456-7890 </a>
         <a href="#"> <i class="fas fa-phone"></i> +111-222-3333 </a>
         <a href="#"> <i class="fas fa-envelope"></i> HAMH@gmail.com </a>
         <a href="#"> <i class="fas fa-map"></i> Karachi, Pakistan - 74200 </a>
      </div>

      <div class="box">
         <h3>follow us</h3>
         <a href="https://www.facebook.com/profile.php?id=100007470010994"> <i class="fab fa-facebook-f"></i> facebook </a>
         <a href="https://twitter.com/hamsafa69362489"> <i class="fab fa-twitter"></i> twitter </a>
         <a href="https://instagram.com/syed_aali?igshid=YmMyMTA2M2Y="> <i class="fab fa-instagram"></i> instagram </a>
         <a href="https://bit.ly/3uCrII8"> <i class="fab fa-linkedin"></i> linkedin </a>
      </div>

   </div>

   <div class="credit"> created by <span>HAM-H</span> | Where We Care! </div>

</section>

<!-- footer section ends -->





<!-- swiper js link  -->
<script src="https://unpkg.com/swiper@7/swiper-bundle.min.js"></script>

<!-- custom js file link  -->
<script src="js/script.js"></script>
<script src="script.js"></script>

</body>
</html>